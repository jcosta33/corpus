...
code review do not affect visible functionality** — they are _evolvability_ defects (structure,
...
template — [`templates/review.md`](https://github.com/jcosta33/corpus-starter-kit/blob/main/templates/review.md) — with a
...
(`corpus review` drafts this packet. It reconciles the finished run against the diff and the spec
...
These are checklist-level (a reviewer inspects them) and toolable. `corpus check` flags the
mechanical ones — empty evidence, coverage, out-of-scope edits. `corpus review` drafts the packet
...
5. **Separate an environment baseline from a feature regression.** A check that fails for
   ...
   environment is Blocked) is recorded as _diagnostic evidence_. The target-environment
...
7. **An agent that runs the app produces evidence, not a verdict.** Driving the live app to exercise
   ...
   run never _certifies_ a change. The author-agent **attaches its run** (accessibility snapshot,
...
[`adversarial-review`](https://github.com/jcosta33/corpus-starter-kit/blob/main/.agents/skills/adversarial-review/SKILL.md)
...
- if you authored the change, you don't decide its review result. Self-review before finishing is
  good practice — the [task template](https://github.com/jcosta33/corpus-starter-kit/blob/main/templates/task.md) asks the
...
- **Participation is the gate, not the sign-off.** A `Pass` requires evidence of _substantive
  engagement_ — a finding raised, a check re-run, a reasoned result. Lax participation, not
...
2. **Write the spec as an acceptance bar** — what must be true for this change to merge, one
   `AC-NNN` per behavior, written _after_ the code exists. It bounds the review, not the build.
...
When the work was delegated to a worker agent, all it hands back about _how_ it ran is its `## Run
...
- **AC-001, the worker wrote `Pass` and cited `npm test -- auth.spec.ts`.** Re-run that command
  yourself in the worktree. Passes → the row is **Pass**, and _your_ pasted output fills the
...
- **Provenance is evidence, not a pass.** If the run records _how_ it was isolated (its own
...
The rule underneath: a delegated run gives you a richer _claim_ to check, never a result you can
