...
| Parser / CLI / LSP | **Out of scope; contracts only.** No parser, linter, planner, scheduler, differ, or language server is shipped or required. The framework defines only the *contracts* such tools MUST satisfy so future tooling can target them (the NO RUNTIME invariant). |
