...
| **No resumable trail** | The session ends mid-stride; the next session starts from scratch and re-discovers the same things. | State is externalised to durable artifacts — sources, status, traces, reviews — rather than held in a conversation [[SCRATCHPAD]](research/sources.md#SCRATCHPAD). (See [the evidence on externalising state to files](./research/task-files.md).) |
