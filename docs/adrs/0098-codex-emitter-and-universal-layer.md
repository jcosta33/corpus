...
[ADR-0092](./0092-swarm-agents-member.md) founded swarm-agents as a **Claude-Code-first**
agent-definition catalog and held a portable layer behind a "do-not-found" gate (needs ≥2 runners
demonstrating value). The prior deep-research survey (swarm-hq) found the gate's second-runner question
...
1. **Codex TOML emitter — `swarm agents emit --codex`** (swarm-cli). Reads the swarm-agents
   ...
   so the reconcile-only posture holds ([ADR-0077](./0077-swarm-cli-reconcile-only-harness.md)). It is a
   **runner adapter** (like `swarm run`'s launcher), so it lives in swarm-cli's Workspace leaf, NOT the
   agent-agnostic Core — naming a runner is exactly what Core's boundary forbids.

2. **The universal `AGENTS.md` discipline layer** (swarm-agents). The shared discipline — evidence over
   ...
   ([ADR-0063](./0063-honesty-framework-and-tooling-boundary.md)) — is single-sourced in swarm-agents'
...
3. **Enforcement does not travel — honest scope** ([ADR-0063](./0063-honesty-framework-and-tooling-boundary.md)).
   ...
   and the universal layer: **toolable** (the tool is `swarm agents emit`). The portability claim is
...
5. **The measurement wave stays the honest exception** (ADR-0092's gate). Demonstrating value across ≥2
   *real external* runner teams is un-fabricatable; it remains a standing owner-run activity, recorded
   as the named exception — not a build item.

## Consequences

- swarm-cli gains the `agents` command (`emit --codex`), a `codexToml` renderer + `emit_agents` engine
...
- swarm-agents' `AGENTS.md` gains a **Portability** section (the universal layer + what does/doesn't
...
- The Claude-Code-first founding stance is preserved: Claude Code reads the `agents/*.md` natively; the
  emitter is an *addition* for a second runner, not a re-platforming.
