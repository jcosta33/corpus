...
- **Language version** — the SOL + APS feature set. Carried **per file** in frontmatter as `corpus_language` (the SOL discriminator, e.g. `SOL/0.1`) and `aps_version` (e.g. `0.1`), so one repo MAY hold `spec.md` files at different language versions mid-migration (§25.1.1).
- **Framework / package version** — the kernel payload, templates, pass guides, profiles, flow-graph. A single semver in `starter-kit/.agents/.corpus-version`, mirrored by an adopted project as `.agents/corpus.version` (§25.1.2). This is exactly [0015](./0015-versioning-scheme.md)'s field, unchanged.
...
- Adds: the language version axis (`corpus_language`, `aps_version`, per-file) and the one-way language⇒framework release trigger (§25.1–§25.2); the three distinct version fields in the IR/plan (§25.3).
