...
[the large-PR-review example](../examples/large-pr-review.md) — your _worked reference_, the
...
owns the _why_ and _when_ of this step; read it once. The shape of the file is frozen in the kit
template — copy
[`templates/intake.md`](https://github.com/jcosta33/corpus-starter-kit/blob/main/templates/intake.md)
...
> The optional `corpus pull` captures this snapshot for you. By hand, you copy-paste into the
...
[`templates/spec.md`](https://github.com/jcosta33/corpus-starter-kit/blob/main/templates/spec.md)
...
- The `Non-goals` block names the things a reader might _assume_ you're changing (the lifetime,
...
[Writing specs](../04-writing-specs.md#writing-rules) calls it _the highest-value line in the
file_, because a runnable check outperforms prose plans as task input. For `AC-001` that line is
...
including the rule that _every_ requirement carries a `Verify with:` line — lives in
[the checks reference](../reference/checks.md). You don't need it for one requirement, but it is
the page reviewers (and the optional `corpus check`) work from, so know it's there.
...
Two files, and a clean separation between them: `intake/checkout-expiry.md` holds _what was
asked_, and `specs/checkout/spec.md` holds _what you'll build and how you'll know it works_.
