...
[the template](https://github.com/jcosta33/corpus-starter-kit/blob/main/templates/task.md) perfectly and still pull a requirement its
...
says so and names the corpus-cli command.
...
altered is a failure — an ugly, rambling ticket captured exactly is a _passing_ intake. P3 cuts
...
[[HILBENCH]](../research/sources.md#HILBENCH) — which is why S3 fails a spec for _burying_ an
...
Not graded here: whether the requirements are _satisfiable_, and whether the eventual change
meets them — those surface at the Run and Review bars. The form half of S1 is toolable —
corpus-cli's `corpus check` flags a requirement with no
...
forbids _silent_ ones — declared, they become a Human attention row at Review.
...
and computing the gate is toolable — a future `corpus review` in corpus-cli.
...
when Y") and check each reached a finding — _and_ check that what reached `findings/` is
...
- [Templates](https://github.com/jcosta33/corpus-starter-kit/tree/main/templates/) — the frozen artifact formats every bar reads against.
